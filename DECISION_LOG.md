...
- **Decision:** Android-only FOSS stack, package `org.astroalarm`, JSON alarm store, bundled city catalog, Hilt/Room on classpath. ADRs 0001 and 0003.
- **Alternatives considered:** Keep OpenShouter as a module (rejected: product is standalone). Remote ephemeris API (rejected: offline-first FOSS).
- **Consequences:** Web/Python/Node examples pruned. Social-preview and go-gate checks skip missing stacks.
