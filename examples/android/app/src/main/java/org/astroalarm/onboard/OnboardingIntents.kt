...
object OnboardingIntents {
    ...
    fun openRingFix(context: Context) {
        val missing = OnboardingChecker.missingRingSteps(context)
        val step = RingWillNotFire.primary(missing) ?: return
        when (step) {
            OnboardingStep.Notifications -> openNotificationSettings(context)
            OnboardingStep.ExactAlarms,
            OnboardingStep.FullScreenIntent,
            OnboardingStep.Battery -> openSpecialAccess(context, step)
            OnboardingStep.Location -> Unit
        }
    }
