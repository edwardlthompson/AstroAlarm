...
if should_run android && [ -f examples/android/gradlew ]; then
  if ! command -v java >/dev/null 2>&1 && [ -z "${JAVA_HOME:-}" ]; then
    ...
    run_in_dir examples/android android-test ./gradlew $gradle_extra test --parallel --no-build-cache
  fi
fi
...
if should_run go && [ -f examples/go/go.mod ]; then
  ...
  echo "Skipping go gate (examples/go missing)"
fi
...
if [ "$STRICT" = true ] && [ "$STACK" = "multi" ]; then
  ...
  if [ -d examples/web/src ]; then
    ...
    log "Skipping about-feature-gate (examples/web missing)"
