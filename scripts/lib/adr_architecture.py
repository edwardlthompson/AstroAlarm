...
def check_repo(root: Path) -> list[str]:
    # Product children accept a pattern; the open-pick gate is for the template only.
...
def main() -> int:
    ...
    if errors:
        ...
        for item in errors:
            print(f"  {item}")
        return 1
    if not is_template_repo(Path.cwd()):
        print("ADR-0001 child product decision present (open-pick gate skipped)")
    else:
